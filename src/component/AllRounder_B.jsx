...
const AllRounder_B = () => {
  ...
  useEffect(() => {
    fetch("./allrounder.json")
      .then((res) => res.json())
      .then((data) => {
        const groupedPlayers = groupPlayersByCategory(data?.allrounder_b);
  ...
  const HandleRandomNumber = () => {
    ...
    if (!hasRemainingPlayers) {
      alert("There are no more All Rounders.");
    }
  };
    ...
    <div className="w-full flex justify-between items-center mx-auto text-center">
      <div className="w-full">
        ...
        <div className="flex justify-between items-center border-b-2 p-0 border-gray-300 w-full">
          <h2 className="text-4xl text-center p-4 pb-0 text-white">All Rounders</h2>
          <button
