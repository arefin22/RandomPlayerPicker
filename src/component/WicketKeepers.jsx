...
const WicketKeepers = () => {
  ...
  useEffect(() => {
    fetch("./wicketkeeper.json")
      .then((res) => res.json())
      .then((data) => {
        const groupedPlayers = groupPlayersByCategory(data?.wicketkeeper);
  ...
  const HandleRandomNumber = () => {
    ...
    if (!hasRemainingPlayers) {
      alert("There are no more Wicket Keepers.");
    }
  };
    ...
    <div className="w-full flex justify-between items-center mx-auto text-center">
      <div className="w-full">
        ...
        <div className="flex justify-between items-center border-b-2 p-0 border-gray-300 w-full">
          <h2 className="text-4xl text-center p-4 pb-0 text-white">WicketKeepers</h2>
          <button
